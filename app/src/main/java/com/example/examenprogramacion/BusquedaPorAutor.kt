...
import com.example.examenprogramacion.databinding.ActivityBusquedaPorAutorBinding
import com.example.examenprogramacion.databinding.ActivityMainBinding
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class BusquedaPorAutor : AppCompatActivity() {

    lateinit var binding: ActivityBusquedaPorAutorBinding
    private lateinit var retrofitService: ConsumirApi
    private lateinit var libroAdapter: LibroAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityBusquedaPorAutorBinding.inflate(layoutInflater)
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        searchView.queryHint = "Buscar libros por autor"

        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String?): Boolean {
                buscarLibrosPorAutor(query ?: "")
                return true
            }

            override fun onQueryTextChange(newText: String?): Boolean {
                buscarLibrosPorAutor(newText ?: "")
                return true
            }
        })
    ...
    private fun buscarLibrosPorAutor(autor: String) {
        if (autor.isNotEmpty()) {
            val call = retrofitService.searchLibrosPorAutor(autor)
            call.enqueue(object : Callback<Libro> {
                override fun onResponse(call: Call<Libro>, response: Response<Libro>) {
                    if (response.isSuccessful) {
                        ...
                        if (libros?.items?.isNotEmpty() == true) {
                            ...
                            Toast.makeText(this@BusquedaPorAutor, "No se encontraron libros con ese autor.", Toast.LENGTH_SHORT).show()
                        }
                    } else {
                ...
                override fun onFailure(call: Call<Libro>, t: Throwable) {
                    libroAdapter.clearData()
                    Toast.makeText(this@BusquedaPorAutor, "Error al consultar la API: ${t.message}", Toast.LENGTH_SHORT).show()
